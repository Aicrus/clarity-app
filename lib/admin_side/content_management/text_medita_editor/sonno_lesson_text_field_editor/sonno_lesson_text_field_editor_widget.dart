...
import 'sonno_lesson_text_field_editor_model.dart';
export 'sonno_lesson_text_field_editor_model.dart';

class SonnoLessonTextFieldEditorWidget extends StatefulWidget {
  const SonnoLessonTextFieldEditorWidget({
    super.key,
    required this.docSon,
  });

  final SonnoRecord? docSon;

  @override
  State<SonnoLessonTextFieldEditorWidget> createState() =>
      _SonnoLessonTextFieldEditorWidgetState();
}

class _SonnoLessonTextFieldEditorWidgetState
    extends State<SonnoLessonTextFieldEditorWidget> {
  late SonnoLessonTextFieldEditorModel _model;
  ...
  void initState() {
    super.initState();
    _model = createModel(context, () => SonnoLessonTextFieldEditorModel());

    _model.titleController ??=
        TextEditingController(text: widget.docSon?.title);
    _model.titleFocusNode ??= FocusNode();

    _model.subTitleController ??=
        TextEditingController(text: widget.docSon?.susubtitle);
    _model.subTitleFocusNode ??= FocusNode();

    _model.indexController1 ??= TextEditingController(text: widget.docSon?.day);
    _model.indexFocusNode1 ??= FocusNode();

    _model.indexController2 ??=
        TextEditingController(text: widget.docSon?.length);
        ...
        children: [
          ...
          Row(
            ...
            children: [
              FFButtonWidget(
                onPressed: () async {
                  logFirebaseEvent('SONNO_LESSON_TEXT_FIELD_EDITOR_DISMISS_B');
              ...
              FFButtonWidget(
                onPressed: () async {
                  logFirebaseEvent('SONNO_LESSON_TEXT_FIELD_EDITOR_SAVE_BTN_');
                  logFirebaseEvent('Button_backend_call');

                  await widget.docSon!.reference.update(createSonnoRecordData(
