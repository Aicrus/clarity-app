...
class FirebaseAuthManager extends AuthManager
    ...
    try {
      ...
      if (e.code == 'requires-recent-login') {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
              content: Text(
                  'Accedi nuovamente per procedere alla cancellazione dell\'account')),
        );
        return false;
      }
    }
    ...
    try {
      ...
      if (e.code == 'requires-recent-login') {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
              content: Text(
                  'Accedi nuovamente per procedere all\'aggiornamento dell\'email')),
        );
      }
    }
  }
  ...
  Future resetPassword({
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
            content: Text('Errore: [error]'.replaceAll('[error]', e.message!))),
      );
      return null;
    }
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content: Text('Email di reset password inviata!')),
    );
  }
  ...
  Future<BaseAuthUser?> _signInOrCreateAccount(
    ...
    try {
      ...
      final errorMsg = switch (e.code) {
        ...
        'invalid_response' => 'Risposta OAuth non valida da Apple. Controlla la configurazione.',
        'missing-id-token' => 'Token di identità Apple mancante. Riprova.',
        'missing-authorization-code' => 'Codice di autorizzazione Apple mancante. Riprova.',
        'not_handled' => 'Richiesta di accesso Apple non gestita',
        'apple-signin-unavailable' => 'Apple Sign-In non disponibile su questo dispositivo',
        'unknown_error' => 'Errore sconosciuto durante l\'accesso con Apple',
        // Autres erreurs courantes
        'network-request-failed' => 'Errore di rete. Verifica la tua connessione.',
        ...
        _ => 'Errore: [error]'.replaceAll('[error]', e.message ?? 'Errore sconosciuto'),
      };
      ScaffoldMessenger.of(context).hideCurrentSnackBar();
      ScaffoldMessenger.of(context).showSnackBar(
      ...
      // Gestion des erreurs non-Firebase
      ScaffoldMessenger.of(context).hideCurrentSnackBar();
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Errore imprevisto: ${e.toString()}')),
      );
      return null;
    }
  }
