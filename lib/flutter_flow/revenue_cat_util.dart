...
Future initialize(
  ...
  try {
    ...
    Purchases.addCustomerInfoUpdateListener((info) {
      customerInfo = info;
      print('💰 Customer info updated. Entitlements: ${activeEntitlementIds}');
      
      // Sincronizar com Firestore quando o customerInfo mudar
      if (currentUserReference != null) {
        currentUserReference!.update(createUsersRecordData(
          isPremium: activeEntitlementIds.isNotEmpty,
        )).catchError((error) {
          print('⚠️ Erro ao sincronizar isPremium: $error');
        });
      }
    });
...
Future restorePurchases() async {
  try {
    ...
    // Atualizar Firestore após restaurar compras
    if (currentUserReference != null) {
      await currentUserReference!.update(createUsersRecordData(
        isPremium: activeEntitlementIds.isNotEmpty,
      )).catchError((error) {
        print('⚠️ Erro ao atualizar isPremium após restore: $error');
      });
      print('✅ isPremium atualizado após restore: ${activeEntitlementIds.isNotEmpty}');
    }
  } on PlatformException catch (e) {
